<template>
    ...
    <v-container>
      ...
      <div v-if="showMsg && isConnected">
        <div v-for="m in lmessages" :key="m.id" :class="['d-flex flex-row align-center my-2',  m.yours ? 'justify-end': null]">
        
          <span v-if="m.yours" class="blue--text mr-3"><v-chip color="green" size="x-large">{{ m.text }}</v-chip></span>
          <span v-else class="blue--text ml-3"><v-chip color="blue" size="x-large">{{ m.text }}</v-chip></span>
  ...
  export default {
    ...
    setup() {
      ...
      const sendMessage = async (msg) => {
        ...
        await stringCharacteristics.writeValueWithResponse(
          enc.encode(msg)
        );
